...
export type IndicatorMapOption = {
  id: string;
  label: string;
  shortLabel?: string;
};

export type IndicatorMapLayer = {
  url: string;
  scenarioId?: string;
  periodId?: string;
};

export type IndicatorMapConfig = {
  type: "cog";
  unit?: string;
  defaultRamp?: RampKey;
  displayRange: [number, number];
  classifications?: ClassificationBreak[];
  scenarios?: IndicatorMapOption[];
  periods?: IndicatorMapOption[];
...
] as const satisfies readonly IndicatorMapOption[];

export const CLIMATE_PERIODS = [
  { id: "1981-2010", label: "1981-2010", shortLabel: "1981-2010" },
  { id: "2041-2070", label: "2041-2070", shortLabel: "2041-2070" },
  { id: "2071-2100", label: "2071-2100", shortLabel: "2071-2100" },
] as const satisfies readonly IndicatorMapOption[];

type ClimateScenarioId = (typeof CLIMATE_SCENARIOS)[number]["id"];
type ClimatePeriodId = (typeof CLIMATE_PERIODS)[number]["id"];

function buildCartographyAssetUrl(assetId: string) {
  return `${CARTOGRAPHY_COG_BASE_URL}/${assetId}.tiff`;
}
...
function buildClimateAssetUrl(
  ...
  const fileName =
    scenarioId === "historical"
      ? `${assetId}_${periodId}.tiff`
      : `${assetId}_${periodId}_${scenarioId}.tiff`;
    ...
    layers: [
      {
        scenarioId: "historical",
        periodId: "1981-2010",
        url: buildClimateAssetUrl(config.assetId, "historical", "1981-2010"),
      },
      {
        scenarioId: "ssp370",
        periodId: "2041-2070",
        url: buildClimateAssetUrl(config.assetId, "ssp370", "2041-2070"),
      },
      {
        scenarioId: "ssp370",
        periodId: "2071-2100",
        url: buildClimateAssetUrl(config.assetId, "ssp370", "2071-2100"),
      },
      {
        scenarioId: "ssp585",
        periodId: "2041-2070",
        url: buildClimateAssetUrl(config.assetId, "ssp585", "2041-2070"),
      },
      {
        scenarioId: "ssp585",
        periodId: "2071-2100",
        url: buildClimateAssetUrl(config.assetId, "ssp585", "2071-2100"),
      },
    ],
  };
}
...
export const Indicators: Indicator[] = [
  ...
  },
  {
    ...
    references: [
      ...
      "Laaich, H. et al. (2016). Soil erodibility mapping using three approaches in the Tangiers province Ã¢â‚¬â€œNorthern Morocco. International Soil and Water Conservation Research (4), 159Ã¢â‚¬â€œ167.",
      "Panagos, P. et al. (2015). Estimating the soil erosion cover-management factor at the European scale. Land Use Policy (48), 38Ã¢â‚¬â€œ50.",
      "Marsoner, T., Simion, H., Giombini, V. et al. A detailed land use/land cover map for the European Alps macro region. Sci Data 10, 468 (2023). https://doi.org/10.1038/s41597-023-02344-3",
      ...
      "Poggio, L., de Sousa, L. M., Batjes, N. H., Heuvelink, G. B. M., Kempen, B., Ribeiro, E., and Rossiter, D., 2021. SoilGrids 2.0: producing soil information for the globe with quantified spatial uncertainty. SOIL, 7, 217Ã¢â‚¬â€œ240. https://doi.org/10.5194/soil-7-217-2021",
      "Fick, S.E. and R.J. Hijmans, 2017. WorldClim 2: new 1km spatial resolution climate surfaces for global land areas. International Journal of Climatology, 37(12): 4302-4315",
  ...
  },
  {
    ...
    methodology: [
      "Annual Net Primary Production (g C/mÂ²/year) was derived from Gross Primary Production using a Carbon Use Efficiency factor of 0.5. The original EEA dataset (2018–2022) was averaged across the five-year period for the study area. The resulting values were standardized to a 0–100 index scale.",
    ],
    references: [
  ...
  },
  {
    ...
    references: [
      "GarcÃ­a, A. M., SantÃ©, I., Loureiro, X. & Miranda, D. Green infrastructure spatial planning considering ecosystem services assessment and trade-off analysis. Application at landscape scale in Galicia region (NW Spain). Ecosystem Services 43, 101115 (2020).",
...
export const getIndicatorsByApp = (app: AppId) =>
  Indicators.filter((indicator) => indicator.apps.includes(app));

export const getIndicatorsWithMap = () =>
  Indicators.filter((indicator) => indicator.map != null);

export const getIndicatorsWithMapByCategory = (category: IndicatorCategory) =>
  Indicators.filter(
    (indicator) => indicator.category === category && indicator.map != null,
  );

export const getIndicatorsWithMapByApp = (app: AppId) =>
  Indicators.filter(
    (indicator) => indicator.apps.includes(app) && indicator.map != null,
  );

export const getIndicatorById = (id: string) =>
  Indicators.find((indicator) => indicator.id === id);

export const getIndicatorMapLayer = (
  indicator: Indicator,
  options: {
    scenarioId?: string;
    periodId?: string;
  ...
  if (!indicator.map) return undefined;

  const { scenarioId, periodId } = options;

  return indicator.map.layers.find((layer) => {
    const scenarioMatches =
      scenarioId == null || layer.scenarioId === scenarioId;
    const periodMatches = periodId == null || layer.periodId === periodId;
    return scenarioMatches && periodMatches;
  });
};
