  ...
  return (
    ...
    >
      ...
      {viewAllHref ? (
        <Link
          href={viewAllHref}
          className="absolute right-4 top-6 z-20 inline-flex items-center justify-center bg-[color:var(--accent)] px-4 py-2 text-sm font-semibold text-white transition hover:bg-[color:var(--accent)] sm:right-8 lg:right-12"
        >
          {viewAllLabel}
        </Link>
      ) : null}
      ...
      <div className="relative z-10 mx-auto flex min-h-[34rem] max-w-7xl items-center px-4 py-16 sm:min-h-[39rem] sm:px-6 lg:min-h-[44rem] lg:px-8">
        <div className="grid h-[28rem] w-full max-w-3xl grid-cols-[2.5rem_minmax(0,1fr)] gap-5 sm:h-[30rem] sm:grid-cols-[3.25rem_minmax(0,1fr)] sm:gap-7 lg:h-[31rem]">
          <div className="flex h-full flex-col items-center pt-1">
            <div className="min-h-0 w-px flex-1 bg-[color:var(--accent)]" />
            <button
              type="button"
              onClick={showNext}
